...
<?php 
	...
	$lblScore 		= Helper::cmsLinkSort('Score ', 'score ', $columnPost, $orderPost);
	...
	// Pagination
	$paginationHTML		= $this->pagination->showPagination(URL::createLink('admin', 'student', 'index'));
                    ...
                    <table class="adminlist" id="modules-mgr">
                        ...
                        <thead>
                            <tr>
								...
								<th class="title"><?php echo $lblScore;?></th>
				...
				<?php
							if(!empty($this->Items)){
								...
								foreach($this->Items as $key => $value){
									...
									$score		= $value['score'];
									$row		= ($i % 2 == 0) ? 'row0' : 'row1';
									$status		= Helper::cmsStatus($value['status'], URL::createLink('admin', 'student', 'ajaxStatus', array('id' => $id, 'status' => $value['status'])), $id);
									$ordering	= '<input type="text" name="order['.$id.']" size="5" value="'.$value['ordering'].'" class="text-area-order">';
									$created	= Helper::formatDate('d-m-Y', $value['created']);
									$created_by	= $value['created_by'];
									$modified	= Helper::formatDate('d-m-Y', $value['modified']);
									$modified_by= $value['modified_by'];
									$linkEdit	= URL::createLink('admin', 'student', 'form', array('id' => $id));
								
		                           	echo  '<tr class="'.$row.'">
		                                	<td class="center">'.$ckb.'</td>
		                                	<td class="center"><a href="'.$linkEdit.'">'.$name.'</a></td>
											<td class="center"><a href="'.$linkEdit.'">'.$score.'</a></td>
