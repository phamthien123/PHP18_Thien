...
<?php 
	...
	// Pagination
	$paginationHTML		= $this->pagination->showPagination(URL::createLink('admin', 'rss', 'index'));
                    ...
                    <table class="adminlist" id="modules-mgr">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th width="10%"><?php echo $lblLink;?></th>
				...
				<?php
							if(!empty($this->Items)){
								...
								foreach($this->Items as $key => $value){
									...
									$link		= $value['link'];
									$row		= ($i % 2 == 0) ? 'row0' : 'row1';
									$status		= Helper::cmsStatus($value['status'], URL::createLink('admin', 'rss', 'ajaxStatus', array('id' => $id, 'status' => $value['status'])), $id);
									$ordering	= '<input type="text" name="order['.$id.']" size="5" value="'.$value['ordering'].'" class="text-area-order">';
									$created	= Helper::formatDate('d-m-Y', $value['created']);
									$created_by	= $value['created_by'];
									$modified	= Helper::formatDate('d-m-Y', $value['modified']);
									$modified_by= $value['modified_by'];
									$linkEdit	= URL::createLink('admin', 'rss', 'form', array('id' => $id));
