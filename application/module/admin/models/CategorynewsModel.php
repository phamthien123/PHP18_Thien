...
class CategorynewsModel extends Model{
	...
	private $_columns = array('id', 'category', 'created', 'created_by', 'modified', 'modified_by', 'status', 'ordering');
	private $_userInfo;
	
	public function __construct(){
		parent::__construct();
		$this->setTable(TBL_CATEGORYNEWS);
	...
	public function listItem($arrParam, $option = null){
		$query[]	= "SELECT `id`, `category`, `status`, `ordering`, `created`, `created_by`, `modified`, `modified_by`";
		$query[]	= "FROM `$this->table`";
		$query[]	= "WHERE `id` > 0";
	...
	public function infoItem($arrParam, $option = null){
		if($option == null){
			$query[]	= "SELECT `id`, `category`, `status`, `ordering`";
			$query[]	= "FROM `$this->table`";
