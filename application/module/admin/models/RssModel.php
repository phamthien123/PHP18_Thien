...
class RssModel extends Model{
	...
	private $_columns = array('id', 'name', 'link', 'created', 'created_by', 'modified', 'modified_by', 'status', 'ordering');
	private $_userInfo;
	...
	public function listItem($arrParam, $option = null){
		$query[]	= "SELECT `id`, `name`, `link`, `status`, `ordering`, `created`, `created_by`, `modified`, `modified_by`";
		$query[]	= "FROM `$this->table`";
		$query[]	= "WHERE `id` > 0";
	...
	public function infoItem($arrParam, $option = null){
		if($option == null){
			$query[]	= "SELECT `id`, `name`, `link`, `status`, `ordering`";
			$query[]	= "FROM `$this->table`";
