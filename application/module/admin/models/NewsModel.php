...
class NewsModel extends Model{
	...
	private $_columns = array('id', 'title','content','picture', 'created', 'created_by', 'modified', 'modified_by', 'status', 'ordering');
	private $_userInfo;
	...
	public function listItem($arrParam, $option = null){
		if($option == null){
			$query[]	= "SELECT `id`, `title`,`content`,`picture`, `status`, `ordering`, `created`, `created_by`, `modified`, `modified_by`";
			$query[]	= "FROM `$this->table`";
			$query[]	= "WHERE `id` > 0";
	...
	public function changeStatus($arrParam, $option = null){
		if($option['task'] == 'change-ajax-status'){
			...
			$result = array(
								'id'		=> $id, 
								'status'	=> $status, 
								'link'		=> URL::createLink('admin', 'news', 'ajaxStatus', array('id' => $id, 'status' => $status))
						); 
			return $result;
		}
	...
	public function infoItem($arrParam, $option = null){
		if($option == null){
			$query[]	= "SELECT `id`, `title`,`content`,`picture`, `status`, `ordering`";
			$query[]	= "FROM `$this->table`";
	...
	public function saveItem($arrParam, $option = null){
		...
		if($option['task'] == 'add'){
			$arrParam['form']['picture']	= $uploadObj->uploadFile($arrParam['form']['picture'], 'news');
		...
		if($option['task'] == 'edit'){
			...
			if($arrParam['form']['picture']['name']==null){
				...
				$uploadObj->removeFile('news', $arrParam['form']['picture_hidden']);
				$uploadObj->removeFile('news', '60x90-' .  $arrParam['form']['picture_hidden']);
